...
public class MixedResults {
  ...
  public static void main( String[] args ) {
    ...
    System.out.println( "Proble 1: " + (57.2 * (i1/i2) + 1) ); // 1.0
    System.out.println( "Proble 2: " + (57.2 * ((double)i1/i2) + 1) ); // 39.1333333333333
    System.out.println( "Proble 3: " + (15 - i1 * ( d1 * 3 ) + 4) ); // -1345.3999999999
    System.out.println( "Proble 4: " + (15 - i1 * (int)(d1 * 3) + 4) ); // -1337
    System.out.println( "Proble 5: " + (15 - i1 * ((int)d1 * 3) + 4) ); // -1313
    ...
    System.out.println ( "Exercise 12-15" );
   ...
   /*
    * Math class methods:
    * Math.abs();    = Absolute value (int, double)
    * Math.pow(x,y); = Exponent (x to the power of y)
    * Math.sqrt(x);  = Square root (double)
    * Math.ceil(x);  = Ceiling (the next highest whole number [ex] 1.1 = 2)
    * Math.floor(x); = Floor (the next lowest number [ex] 1.9 =1)
    * Math.min(x,y); = Minimum of x or y
    * Math.max(x,y); = Maximum of x or y
    * Math.randon(); = Returns a random number between 0<=x<1
    ...
    */
     ...
     * Math.asin(x);       = arcsine of in range -PI/2 to PI/2
     ...
     * Math.toRadians(x);  = converts degress to radians
     */
